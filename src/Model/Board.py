...
class Board:
    ...
    def is_move_legal(self, move):
        ...
        if self.phases[move.color] != 0 and self.nodes[move.from_pos] != move.color:
            ...
            return False
        if self.phases[move.color] == 1 and not self.are_positions_adjacent(move.from_pos,
                                                                            move.to_pos) and move.to_pos != -1:
    ...
    def check_win(self):
        ...
        if (len(self.phase_one_get_legal_moves(0)) == 0 and self.phases[0]==1) or (len(self.phase_one_get_legal_moves(1)) == 0 and self.phases[1] == 1):
            return True
        # mindestens ein Spieler muss in der letzten Phase sein
        if not self.phases[0] == 2 and not self.phases[1] == 2:
            return False
    ...
    def check_remis(self):
        ...
        if self.last_muehle_counter == 50:
            self.game_end_info = "Remis - 50 Züge ohne Mühle"
            return True
        # 3 mal die gleiche Stellung
        if self.node_states.count(self.nodes) == 3:
            self.game_end_info = "Remis - 3 mal die selbe Stellung"
            return True
        return False
    ...
    def update_game_end(self):
        ...
        if self.check_win() == True:
            ...
            if self.get_num_of_pieces_on_board(1) < 3:
                self.game_end_info = "Weiß gewinnt - Schwarz hat nur noch zwei Steine übrig"
    ...
    def undo(self, steps=1):
        if len(self.node_states) == 1:
            return
        if self.next_move.from_pos != -1 or self.next_move.to_pos != -1:
            self.next_move = Move.Move(self.next_move.color)
        else:
            if self.last_muehle_counter > 0:
                self.last_muehle_counter -=1
            else:
                print("Jetzt sind Remisbedingungen nicht mehr gültig.")
            self.node_states.pop()
            self.nodes = self.node_states[-1]
            self.next_move = Move.Move(self.next_move.color +1)
